...
def show_hospital(df):
    ...
    with c1:
        ...
        <div class='metric-card'>
            <p style='color:#888;margin:0;font-size:0.82rem;'>AI 권장 배정량</p>
            <h2 style='color:#1565c0;margin:0.3rem 0;font-size:1.6rem;'>{allocated:.0f}</h2>
            <p style='color:#aaa;margin:0;font-size:0.78rem;'>Units</p>
        </div>""", unsafe_allow_html=True)

    with c2:
        st.markdown(f"""
        <div class='metric-card'>
            <p style='color:#888;margin:0;font-size:0.82rem;'>예측 수요량</p>
            <h2 style='color:#333;margin:0.3rem 0;font-size:1.6rem;'>{my_row['predicted']:.0f}</h2>
            <p style='color:#aaa;margin:0;font-size:0.78rem;'>Units</p>
        </div>""", unsafe_allow_html=True)
    ...
    with c3:
        st.markdown(f"""
        <div class='metric-card'>
            <p style='color:#888;margin:0;font-size:0.82rem;'>안전재고 여유</p>
            <h2 style='color:#2e7d32;margin:0.3rem 0;font-size:1.6rem;'>+{surplus_pct:.1f}%</h2>
            <p style='color:#aaa;margin:0;font-size:0.78rem;'>예측 대비</p>
        </div>""", unsafe_allow_html=True)
    ...
    with c4:
        ...
        <div class='metric-card'>
            ...
            <h2 style='color:{s_color};margin:0.3rem 0;font-size:1.0rem;white-space:nowrap;'>{status}</h2>
    ...
    with tab1:
        ...
        ax1.plot(recent['date'], recent['actual_use'], color='#2196F3', lw=2, label='Actual Use')
        ...
        ax1.axhline(allocated, color='#4caf50', lw=2, linestyle='-.',
                    label=f"Today Allocation ({allocated:.0f})")
        ax1.set_ylabel('Units')
        ax1.legend(fontsize=8)
        ax1.spines['top'].set_visible(False)
        ax1.spines['right'].set_visible(False)
        ax1.grid(axis='y', color='#eeeeee')
        ax1.set_title('Platelet Demand & Allocation (Last 30 Days)', fontsize=11, fontweight='bold')
    ...
    with tab2:
        ...
        혈액원 담당자가 검토 후 승인합니다.</div>""", unsafe_allow_html=True)
        ...
        if my_reqs:
            ...
            for r in reversed(my_reqs):
                ...
                st.markdown(f"""
                <div style='border-left:4px solid {bd};border-radius:8px;
                padding:0.7rem;margin:0.3rem 0;font-size:0.88rem;background:white;'>
